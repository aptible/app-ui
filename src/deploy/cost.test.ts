...
import { backupCostPerGBHour, calculateCost, diskCostPerGBMonth, diskIopsCostPerMonth, endpointCostPerHour, hoursPerMonth, stackCostPerMonth, vpnTunnelCostPerMonth } from "./cost"

describe("calculateCost", () => {
  it("should calculate the cost of containers", () => {
    const { hourlyCost, monthlyCost } = calculateCost({
      services: [
        { "containerCount": 2, "containerMemoryLimitMb": 2048, "instanceClass": "r5" },
        { "containerCount": 1, "containerMemoryLimitMb": 1024, "instanceClass": "r5" },
      ]
    })

    expect(hourlyCost).toBeCloseTo(5 * CONTAINER_PROFILES.r5.costPerContainerGBHourInCents / 100)
  ...
  it("should calculate the cost of disks", () => {
    ...
    expect(monthlyCost).toBeCloseTo(5 * diskCostPerGBMonth + 1000 * diskIopsCostPerMonth)
  })
  ...
  it("should calculate the cost of everything", () => {
    ...
    let expectedHourly = 5 * CONTAINER_PROFILES.r5.costPerContainerGBHourInCents / 100
