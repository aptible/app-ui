...
export const Sources = () => {
  ...
  // Loading states
  const isSourcesInitialLoad =
    sources === null || sourcesWithLiveCommits === null;
  ...
  const isSourcesLoading =
    isSourcesInitialLoad || isForceLoading || showSkeleton;

  // Only compute sourceWithMostLiveCommits when all loading is complete
  const sourceWithMostLiveCommits =
    !isSourcesInitialLoad && !isForceLoading && !showSkeleton
      ? sourcesWithLiveCommits?.[0]
      : null;
  ...
  const isSourcesEmpty =
    ...
    !isForceLoading &&
    !showSkeleton &&
    (!sources?.length || !sourcesWithLiveCommits?.length);
              ...
              {isSourcesLoading ? (
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex gap-1">
                    {[...Array(2)].map((_, i) => (
                      <div
                        key={i}
                        className="h-4 w-1.5 bg-gray-200 rounded animate-pulse"
                      />
                    ))}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {sourceWithMostLiveCommits?.liveCommits?.length > 0 ? (
                    ...
                    >
                      {sourceWithMostLiveCommits.liveCommits
                        .slice(0, 7)
                        .map((liveCommit) => (
                          <Tooltip
                            key={liveCommit.sha}
