...
const selectRecentActivities = createSelector(
  ...
  (operations, apps, services, databases, state) => {
    ...
    const getResourceInfo = (operation: any) => {
      ...
      const actualResourceType = operation.resourceType;
      ...
      if (operation.resourceType === "app") {
        ...
        const service = services.find(
          (service) => service.id === operation.resourceId,
        );
...
export const RecentActivity = () => {
      ...
      if (operations.length > 0) {
        ...
        if (activities) {
          const foundActivities = Object.entries(activities)
            ...
            .map(([type, _]) => type);
          console.log(
            `🎯 Activities found by selector: ${foundActivities.join(", ") || "none"}`,
          );
        }
        ...
        if (
          activities &&
          Object.values(activities).some((activity) => activity !== null)
        ) {
  ...
  useEffect(() => {
    ...
    const fetchInitialOperations = async () => {
      ...
      console.log(
        "🚀 Fetching recent operations (mixed strategy):",
      ...
      try {
        ...
        const generalPromises = Array.from(
          { length: generalPages },
          async (_, i) => {
            try {
        ...
        const specificPromises = specificOperationTypes.flatMap((opType) =>
          Array.from({ length: 3 }, async (_, i) => {
            try {
              ...
              console.error(
                `❌ Error fetching ${opType} page ${i + 1}:`,
                error,
              );
              throw error;
            }
        ...
        console.log(
          `📡 All API calls sent (${generalPages + specificOperationTypes.length * 3 + 1} total), waiting for Redux store...`,
        );
      } catch (error) {
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th
                  scope="col"
                  className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[100px] border-b border-gray-200"
                >
                  Type
                </th>
                <th
                  scope="col"
                  className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[160px] border-b border-gray-200"
                >
                  Resource Name
                </th>
                <th
                  scope="col"
                  className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[140px] border-b border-gray-200"
                >
                  User
                </th>
                <th
                ...
                <th
                  scope="col"
                  className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-[120px] border-b border-gray-200"
                >
                  Date
                </th>
              </tr>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {(
                ["deploy", "apps:scale", "db:restart", "Failed Op"] as const
              ).map((activityType) => {
                const operationData =
                  !isLoading && memoizedActivities
                    ? memoizedActivities[activityType]
                    : null;
                  ...
                  <tr key={activityType} className="hover:bg-gray-50">
                      ...
                      {isLoading ? (
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-gray-500">
                            {operationData.resourceType
                              .charAt(0)
                              .toUpperCase() +
                              operationData.resourceType.slice(1)}
                          </span>
                        </div>
                      ) : (
                    ...
                    <td className="px-6 py-3 text-sm text-gray-500">
                      {isLoading ? (
                        ...
                        <span className="break-all block w-full">
                          {operationData.user}
                        </span>
                      ) : (
