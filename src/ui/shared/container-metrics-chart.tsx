...
import { Dataset, MetricHorizons, processDataSeries } from "./metrics-controls";
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
      {chartsToCreate.map((chartToCreate, idx) => (
        <div
          key={`${chartToCreate.title}-${idx}`}
          className="bg-white px-5 pt-1 pb-5 shadow rounded-lg border border-black-100 relative min-h-[400px] bg-[url('/thead-bg.png')] bg-[length:100%_46px] bg-no-repeat"
        >
          <LineChartWrapper keyId={idx.toString()} chart={chartToCreate} />
        </div>
      ))}
    </div>
  );
