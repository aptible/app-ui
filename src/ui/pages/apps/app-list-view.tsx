...
export const AppListView = ({ apps }: { apps: DeployApp[] }) => {
  ...
  function onCancelCreateAppModal() {
    setIsCreateAppModalOpen(false);
  }

  function onSubmitCreateAppModal() {
    // Save an app
    setIsCreateAppModalOpen(false);
  }
  return (
    <>
      <CreateAppModal
        onSubmit={onSubmitCreateAppModal}
        onCancel={onCancelCreateAppModal}
      ...
      <ResourceListView
        ...
        actions={[
          <Button type="button" variant="primary" onClick={showCreateAppModal}>
            Create App
          </Button>,
        ]}
        tableHeader={
