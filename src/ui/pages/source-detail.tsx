...
import { useLoader, useQuery } from "starfx/react";
import {
  AppListBySource,
  BannerMessages,
  Box,
  ButtonOrgOwner,
  DeploymentsTableBySource,
  FormGroup,
  Group,
  Input,
} from "../shared";
...
export function SourceDetailDeploymentsPage() {
  const { id = "" } = useParams();
  useQuery(fetchDeploymentsBySourceId({ id }));
  const deployments = useSelector((s) =>
    selectDeploymentsBySourceId(s, { sourceId: id }),
  );
  return <DeploymentsTableBySource deployments={deployments} />;
}
